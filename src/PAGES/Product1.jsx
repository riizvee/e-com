...
export default function Product1() {
  ...
  const addToCart = (photo) => {
    let curentdataRaw;
    try {
      curentdataRaw = JSON.parse(localStorage.getItem('cart'));
    } catch (e) {
      curentdataRaw = null;
    }

    const curentdata = Array.isArray(curentdataRaw) ? curentdataRaw : [];

    const existitem = curentdata.find((itm) => itm.id === photo.id);

    if (existitem) {
      existitem.quantity = (existitem.quantity ?? 0) + 1;
    } else {
      curentdata.push({ ...photo, quantity: 1 });
    }

    localStorage.setItem('cart', JSON.stringify(curentdata));
  ...
  return (
    <div className="min-h-screen bg-black p-10 flex flex-col justify-center items-center">
      <h1 className="text-white text-2xl mb-6">PRODUCT PAGE 1</h1>
